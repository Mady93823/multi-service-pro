...
### D36 — A city is a grouping over zones, and its timezone is load-bearing (2026-07-15)

`M25` had an easy shape available: keep `zones.city` as a string, add a `cities` table for the switcher, and let the two drift. It was refused. The string had no owner — two zones could spell one town differently and nothing could hang off either spelling — so the migration **backfills it into rows and drops the column**. `zones.city_id` is `restrict`: a city with zones cannot vanish under live bookings.

**The city gate is the zone gate one level up, never a second geography.** The catalog and dispatch have been zone-gated since M03/M06, and all membership is PHP point-in-polygon (D12). `Service::scopeInCity()` therefore reads the very `service_zone` rows the address pin already resolves against, grouped by `zones.city_id` — no new geometry, no second definition of "we serve you here" that could disagree with the first. A visitor with an address is gated by their **zone**; a guest, or a customer browsing a town they have no address in, is gated by the **city**; `ActiveCity` resolves in one place (explicit session choice → detected from the default address pin → first active city). And a **zone only serves while its city does**, so switching a city off closes the whole town in one click — off the switcher, no longer resolving new address pins, its services gone from the storefront — **while every historical booking keeps its `zone_id`** and its money rows stay the snapshots they always were (D33's rule, applied to geography).

**The timezone is the part worth arguing about.** A city that carries a timezone nobody reads is a lie in the schema, and the honest options were to drop the column or to make it decide something. It decides the slot grid: "we work 08:00–20:00" is a claim about the town the visit happens in, not about the server. `SlotGenerator` already had exactly one seam for this (a private `timezone()`, with slot values travelling as UTC ISO strings), so `days(?City)` / `isBookable($utc, ?City)` fall out of it — the platform timezone stays the fallback when there is no city, which is every single-city install and every guest. The checkout re-asks for the grid when the customer picks an address in another city and drops the slot they had chosen: it was a different wall-clock hour. Everything else — `scheduled_at`, expiry, tracking, cron — keeps storing absolute instants; **the slot grid is the one place a wall clock is read**, and that is exactly why per-city time cost one parameter instead of a second time model.

### D37 — Security lives where the request is not (2026-07-15)

Sixteen modules of policies, form requests and route guards all answer the same question: *may this user do this?* The Phase 7 pass found that every real hole was somewhere else — in the space **outside** the request a policy sees.

**The framework was serving the private disk.** The starter ships `'serve' => true` on the `local` disk, which is the disk that holds KYC documents, bank receipts and ticket attachments. It registers `GET /storage/{path}` and — since Laravel 11 — `PUT /storage/{path}`, a signed *write* into that folder. Both are gated by a signature rather than by `BookingPolicy` or `SupportTicketPolicy`; they are a second door those policies have no say over, and no amount of care inside our controllers would have shut it. Every private file already has an authorizing controller, and nothing in the app calls `Storage::disk('local')->url()`, so the setting is `false` and a test asserts the routes are gone. **The rule: a private disk gets exactly one way out, and we own it.**

**A header middleware in the `web` group never runs on the responses that matter.** `Authenticate` is in Laravel's middleware *priority* list, so it is sorted ahead of anything appended to a group: an unauthenticated request is redirected out before our middleware is reached, and the login redirect — along with every 403, 404 and 500 the exception handler renders — ships bare. `SecurityHeaders` is therefore **global**, wrapping the router itself. It sends `nosniff`, `X-Frame-Options: DENY` (an admin panel that can be framed is a Refund button that can be clicked), `Referrer-Policy`, and a `Permissions-Policy` that deliberately keeps `geolocation=(self)` — M07's provider journey screen reads GPS, and a header that quietly withheld it would have killed live tracking with no error anywhere. HSTS only on a secure request in production: sent from a dev machine it would pin the developer's own browser to `https://localhost` with no way back.

**Who may read a file and what the browser may do with it are different questions.** Uploads accept PDFs (KYC, receipts, attachments), and a PDF served inline from our own origin is a script host — so `ServesPrivateFiles` renders images inline and downloads everything else, with `nosniff` closing the other half. Mime validation on the way in is a claim made by the uploader's browser; this is the second lock.

**Sweeps, not samples — and derived, not declared.** The secret-exposure test does not read a list of credentials someone maintains; it derives them from the *key names* in `SettingsRegistry::defaults()`, so a settings group added next year is covered without being told about. Same shape for rate limits: rather than throttling the endpoints we currently think are dangerous, a test fails any state-changing route reachable **without a session** that carries no limiter. That is the class that gets forgotten — the ordinary POST that happens to need no login.
...
### D43 — Cash is offered per geography; online payment is universal (2026-07-18)

Some areas take pay-after-service, some are online-only — so `cash_enabled` (boolean, default `true`) lives on **both** `cities` and `zones`, and cash is offered only where **city AND zone** allow it (`Zone::allowsCash()`). The shape mirrors `Zone::scopeActive` requiring an active city (M25): the city switch closes a whole town's cash in one click, the zone switch handles a single area, and the per-zone flag can never re-open what the city closed. Online methods (gateways, wallet, bank transfer) are **never** gated by geography — only the doorstep-cash risk is local. Enforcement is layered the usual way: `PlaceBookingRequest::availableMethods(?Zone)` filters the `Rule::in` list against the zone of the **posted address** (resolved defensively pre-validation — a cash POST for an online-only address 422s), while the checkout picker filters per selected address from a `cash_allowed` flag riding the existing per-address array (the `blocked_services` idiom) — presentation, not enforcement. No zone (site-wide contexts, out-of-zone addresses) falls back to the global `payments.pay_after_service` flag alone. Existing bookings are untouched: the flag gates what is *offered*, never what was (D33's snapshot rule applied to payment geography). No new settings key — the global pay-after-service flag stays the master switch, so landmine 13 is not in play.

### D44 — Turn-by-turn navigation is a deep link, not a feature (2026-07-18)

The provider's "Navigate" button is a plain Google Maps directions URL (`google.com/maps/dir/?api=1&destination=lat,lng&travelmode=driving`) built from the booking's `address_snapshot` pin — no API key, no SDK, no billing, works with Firebase absent (D4/D14's spirit). The **origin is deliberately omitted**: Maps then routes from the device's own live GPS, which is always fresher than our last tracking ping. Our Leaflet map keeps its job (showing the *customer* where the provider is); routing belongs to the app the provider already trusts. Shown on the journey screen and the accepted/en-route job card only. The same pass fixed the payload behind the privacy line D41 drew: `DispatchOfferResource` used to embed the full `BookingResource` — street line, exact pin, phone — while the offer card *rendered* only the city. Inertia serializes every prop into the page HTML, so "the card only shows the city" was presentation, not privacy. The offer payload is now a trimmed shape (items, schedule labels, total, `address.city`) and a test pins that an open offer never carries the doorstep.
