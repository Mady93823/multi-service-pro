...
- **Shipped (2026-07-18, D43):** `cash_enabled` on **cities and zones** (default on) — pay-after-service is offered only where city AND zone allow it (`Zone::allowsCash()`); online methods are never geography-gated. Checkout shows a per-address `cash_allowed` flag and swaps the picker when the customer selects an online-only address; `PlaceBookingRequest` re-validates against the posted address's zone, so a cash POST for an online-only area 422s. Admin toggles on the zone + city forms, "Online only" badge on both lists.
...
- **Shipped (2026-07-18, D44):** "Navigate with Google Maps" on the journey screen + accepted/en-route job card — a plain directions deep link from the `address_snapshot` pin (no API key, origin omitted so Maps routes from the device's live GPS). Same pass trimmed `DispatchOfferResource`: an **open offer's payload** now carries city-only (no street line, no pin, no phone) — the card always rendered city-only, but Inertia serializes props into the page HTML, so the payload is the privacy boundary (test-pinned).



## M08 Payments & Wallet
- Gateway abstraction `PaymentGateway` interface → `RazorpayGateway` (**default — client confirmed**; UPI-first payment method ordering), `PayUGateway` (second Indian rail, D39), `StripeGateway` (product requirement for non-India buyers, D8), `PayPalGateway` (international installs, D39 — cannot settle INR), `CashGateway` (pay-after-service, enabled at launch)
...
- **Shipped (2026-07-17, D39):** `PayUGateway` (hosted-form checkout — signed form out, reverse-hash cross-site POST back on the unauthenticated CSRF-exempt `payments/payu/return`, `verify_payment` API re-check before settle, form-encoded webhook) and `PayPalGateway` (Orders v2 — approve redirect, `captureOrder()` as the only money-mover on both the return leg and the `CHECKOUT.ORDER.APPROVED` webhook, webhook authenticity via the `verify-webhook-signature` API, fails closed without `paypal_webhook_id`). Both in `GatewayManager` display order Razorpay → PayU → Stripe → PayPal; secrets (`payu_salt`, `paypal_client_secret`) write-only; replay-idempotency pinned for all four gateways.
...
- `/install` wizard: requirements check (PHP ver/exts, writable dirs incl. `public/`) → DB credentials → run migrations+seeders **+ `storage:link`** → create admin → write `.env` (incl. generated `REVERB_*` keys) → done
- **Gate = one `.env` line.** `INSTALL=false` (shipped in `.env.example`) opens the wizard; the admin step deletes it. **Absence means installed** — the old `storage/app/installed.lock` meant absence = *not* installed, so a wiped storage directory silently reopened the wizard on a live site. A `.env` that is genuinely missing has no APP_KEY and cannot serve anything, so it degrades to dead, not to open.
- **Copying `.env.example` is the entire prerequisite.** No command runs before the wizard, and nothing the wizard needs may touch the database — see the P7.7 note in `03-Tech-Stack.md` (D38) for why that is a rule and not a nicety.
- The admin step also asks the **database** whether it has been installed (`system.installed_at`, machine state like `system.scheduler_last_run`): `.env` is a file a buyer can restore from backup, and minting an admin over a trading site is the one irreversible thing here. Not "does an admin exist" — ticking demo data seeds one.
...
> [!done] Storage setting shipped 2026-07-17 (ADR D40)
> The public media disk became a **settings choice**: local server, or any S3-compatible bucket (Cloudflare R2 / AWS S3 / Spaces / MinIO) via the new `storage` settings group — `StorageConfigurator` (the `MailConfigurator` idiom) registers a `s3media` disk at boot and points `media-library.disk_name` at it when fully configured; anything less degrades to local (D35). **Switching affects new uploads only** — every media row keeps the disk it was written to, so nothing strands (pinned). Private collections keep their explicit `local` pin and never move. Secret write-only; admin Test-connection button probes the stored settings synchronously (write → read back → delete). New dep: `league/flysystem-aws-s3-v3`. 7 new tests (833 suite-wide, 4809 assertions).
